...
def build(universe: bool = False) -> pd.DataFrame:
    """`universe` keys the cases on the reconciled universe (two-digit years expanded, as
    `acquire_external_data.case_universe` defines it) instead of the printed number this
    memo was computed on; `python analyze_delay.py universe` reports both."""
    it = load()
    it["sd"] = supervisor_districts(it)
    it["cont_in"] = arrived_continued(it)
    if universe:
        from acquire_external_data import load_items
        it = it.merge(load_items()[["item_id", "cn"]].rename(columns={"cn": "ucn"}),
                      on="item_id", how="left")
        it["ucn"] = it.ucn.fillna("")
    d = it.copy()
    d["cn"] = d.ucn if universe else \
        d.case_number.astype(str).str.upper().str.replace(r"\s+", "", regex=True)
    d = d[d.cn.ne("")]
    ...
    X = chains(it, key="ucn" if universe else None).join(first[["preliminary_recommendation_category", "type_district",
                               "speakers", "sd", "cont_in"]])
...
def universe_macros():
    """The reconciliation in this memo's terms (2026-09-11): the case counts, the estimation
    sample and its returning shares, keyed both ways. The printed-number figures must
    reproduce the memo's; the others are what the reconciled universe gives."""
    n = lambda x: f"{x:,}".replace(",", "{,}")
    M = {}
    for tag, u in (("Printed", False), ("Universe", True)):
        X = build(universe=u)
        S = estimation_sample(X)
        M.update({f"delayCases{tag}": n(len(X)), f"delaySample{tag}": n(len(S)),
                  f"delayMulti{tag}": f"{100*(S.hearings > 1).mean():.1f}",
                  f"delayPositive{tag}": f"{100*(S.days > 0).mean():.1f}"})
    (TAB / "delay_macros.tex").write_text(
        "% GENERATED BY analyze_delay.py universe --- do not edit by hand.\n" +
        "\n".join(rf"\newcommand{{\{k}}}{{{v}}}" for k, v in sorted(M.items())) + "\n")
    print(M, "→", TAB / "delay_macros.tex")
