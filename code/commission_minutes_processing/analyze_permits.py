...
def _parcel_keys(r) -> set[str]:
    ...
    if not b or not lots:
        return set()
    return {f"{b.zfill(4)}:{str(l).strip().zfill(3)}" for l in lots}
...
def write_tables(it, pr, m, rules, reach, cond, byform, bydec, bytype, buys):
    ...
    a(r"\caption{Permit numbers as the minutes print them, and whether DataSF's Building "
      r"Permits table (\texttt{i98e-djp9}, 1{,}295{,}048 rows) holds them. Distinct "
    ...
    a(r"Items & Count & Share of the 16{,}199\\\midrule")
    n_all = len(it)
...
def main():
    ...
    write_tables(it, pr, m, rules, reach, cond, byform, bydec, bytype, buys)
