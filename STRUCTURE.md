...
| `analyze_corpus.py` | The corpus memo, generated: composition, dispositions, delay chains (`chains()`, reused downstream), commissioners, geography, Planning Code citations → `discretionary_review_patterns/{figures,tables}`. `python analyze_corpus.py universe` writes `tables/corpus_macros.tex`: the memo's case counts recomputed on the printed numbers (they must reproduce) and on the reconciled universe, for its dated note. |
...
| `analyze_conditions.py` | The conditions memo: `probe` (is a Commission packet published for this case?), `fetch` (pull it, keep only the motion's Exhibit A), `report` (coverage, a block-text sweep, and a thirteen-category taxonomy; plus `tables/condition_macros.tex`, the figures its dated corrections quote, read from `acquisition_summary.json` and the census's `census_summary.json`). Both network stages cache to `$MFHR_DATA_ROOT/external/cpc_packets/`. Superseded as a source of coverage by `collect_conditions.py`. |
| `collect_conditions.py` | **The conditions census** (Task 1 of the 2026-09-10 brief) and the conditions-content memo. Stages, each cached and resumable: `discover` (directory listings, Wayback CDX manifests, the 2015– hearing pages, the archived 1998–2014 agenda pages, and every document hyperlink in the raw minutes), `probe` (HEAD every case and motion number against every URL family, ≤2 req/s/host → `availability_full.csv`), `pull` (one process per host; keep adopted motions whole and only the motion section of a packet; OCR only image-only pages inside a motion), `reextract` (pages read before `/Rotate` was honoured), `parse` (→ `conditions_long.{parquet,csv}`, `motion_sections.csv`), `diff` (draft against adopted), `gold` / `freeze` / `score` / `archive` / `sample` (the frozen validation rounds; round 3 current), `summary` (`task1_summary.md`), `numbers` (`conditions_numeric.parquet`: the numbers inside the conditions), `report` (`conditions_content/{figures,tables}`, `census_summary.json`), `records` (the draft records request → `records_request/`). |
| `condition_parser.py` | The rules that turn a Commission document into conditions — motion sections, the Exhibit A and its end, condition boundaries, headings, running headers and compliance lines — kept apart from `collect_conditions.py` so they can be hashed and frozen for a validation round (`validation/FROZEN_ROUND*.json`). Do not edit it while a round is being labelled. |
| `analyze_permit_content.py` | The permits-content memo: `fetch` pages DBI's Building Permits in full by `:id` (all 57 columns, 1.3M rows), `report` inventories every field from the rows, the permit clock, a development-relevant universe, linkage tiers T1/T2/T3, linked-vs-unlinked comparisons (raw, exact-cell reweighted, within unit bins, and landmark-anchored from the hearing), the DR/CU/ministerial comparisons, bunching at thresholds and the DR developer clock → `permits_content/{figures,tables}`. `clocks` writes the clocks memo (Kaplan–Meier, hazards and full distributions of every duration by track, value, size, period and neighbourhood → `external/clocks/`); `outcomes` adds its outcome chain (entitlement → issuance → completion → units, with exit as a competing event). |
| `analyze_datasf_catalogue.py` | The DataSF catalogue memo: `fetch` pages the Socrata Discovery API for `q=planning` by `dataset_id` (relevance order is unstable), tests both host names and both parameter forms, reads Planning's own data inventory and enriches each asset with its view metadata and `count(*)`; `report` writes `datasf_planning_catalogue/tables` from those and the hand-written judgement flags in `relevance.csv`. Raw JSON under `$MFHR_DATA_ROOT/external/datasf_catalogue/`. |
| `build_exaction_panel.py` | The exactions memo (τ in dollars): `fetch` (the Board's ordinance lists, every cited source into a cache under `$MFHR_DATA_ROOT/external/legal/`, with a guard that refuses an archive capture other than the one cited), `claims` (`exactions/exaction_sources.csv` from `CLAIM_SPECS` and the 2011–2026 fee registers: every rate, date and vesting rule a verbatim quotation), `probe` (re-verify every quotation), `build` (the inclusionary schedule, register rates, fee areas and the new-construction project panel → `external/exactions/`), `report` (memo tables, figures, macros, the cross-check against the conditions' printed percentages). |
| `audit_state_laws.py` | The regulatory-timeline memo: `fetch` / `claims` / `probe` (45 state laws and SF ordinances quoted from leginfo, the Board's lists and the enacted PDFs → `regulatory_timeline/law_{inventory,claims}.csv`), `build` (docket composition by quarter, the permit-side linked share, first-stage series → `external/laws/`), `report`. Shares `build_exaction_panel.py`'s source cache. |
| `build_chronicle.py` | The project chronicle's macros and tables: reads every memo's generated macros, the older memos' JSON artefacts, git history and `run_log.md`, and the data panels' row counts; refuses to run if two memos define the same macro. |
| `analyze_delay.py` | The delay memo: builds the case panel from `chains()`, joins DBI for the supervisor district, and runs OLS / LPM / quantile regressions plus a temporal out-of-sample test. `python analyze_delay.py universe` writes `tables/delay_macros.tex` (the case counts both ways, for its dated note). |
| `acquire_external_data.py` | The data-acquisition memo, in five cached stages: `sync` (materialise Dropbox online-only files — an un-materialised directory is invisible to `ls`), `probe` (resolve every DataSF identifier against the live catalogue), `fetch` (cache the parcel layer, assessor roll, historic zoning, planning records, pipeline, Zillow, the Cotality SF slice and the impact-fee registers under `$MFHR_DATA_ROOT/external/`), `spatial` (point-in-polygon → the 1998–2026 parcel-year zoning panel), `report` (join rates, the risk-set funnel, the price surface, figures, tables, and the `external/README.md` provenance table), `envelope` (the feasible envelope per parcel-year → `external/zoning/parcel_envelope.parquet`, from quoted zoning-table rules). Establishes the filing-to-hearing clock and the `building_permits` bridge that overturns the conditions memo's non-overlap claim. |
...
analyze_corpus | analyze_permits | analyze_delay | analyze_conditions`; then
`acquire_external_data (sync → probe → fetch → spatial → report)`, `collect_conditions
(discover → probe → pull → parse → diff → validation → summary → report)` — whose parse
feeds `acquire_external_data report` (the bridge-by-condition-text overlap) and
`analyze_conditions report` (its dated corrections) — `analyze_permit_content (fetch →
report → clocks → outcomes)` and `analyze_datasf_catalogue (fetch → report)`; then
`build_exaction_panel (fetch → claims → probe → build → report)`, `audit_state_laws (fetch →
claims → probe → build → report)`, `acquire_external_data envelope`, `collect_conditions
records`, and last `build_chronicle` (it reads every other memo's macros).
