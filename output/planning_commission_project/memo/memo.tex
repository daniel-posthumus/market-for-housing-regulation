...
\texttt{clocks/} & The full distribution of every duration by track, parcel value, size,
period and neighbourhood (Kaplan--Meier, hazards), and the outcome chain from entitlement to
units delivered. \\
\texttt{exactions/} & The inclusionary requirement and impact fees that vested for each project,
every figure a verified quotation of its primary source, and a project panel priced in dollars. \\
\texttt{regulatory\_timeline/} & The state laws and ordinances that bear on the project, what
each changed, and whether this data can measure it. An audit, not a causal memo. \\
\texttt{project\_chronicle/} & The whole project in one sitting: every memo with what later work
revised in it, the data inventory, the model, and the open register. \\
\texttt{records\_request/} & A draft records request (not sent) for the pre-2010 motions no
address answered for. \\
