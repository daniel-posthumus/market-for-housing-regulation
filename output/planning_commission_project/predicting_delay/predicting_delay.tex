...
\textbf{16.9\% of the variance} of $\log(\text{days}+1)$ in sample and \textbf{9.6\% out of
...
the same covariates reach \textbf{$R^2=0.054$ in sample and $0.032$ out of sample}. Sorting
...
which carries a district on 99.8\% of its rows --- takes it to 0.128. Where a project is
...
takes $R^2$ from 0.142 to 0.169, with a coefficient of $-0.355$ ($t=-16.4$): \emph{more}
...
$R^2 = 0.169$. On $\mathbb{1}[\text{days}>180]$ it reaches 0.088, and on
$\mathbb{1}[\text{days}>365]$, 0.054. \textbf{Between 83\% and 95\% of the variance is
...
\textbf{The level: 0.174 in sample, 0.096 out.} Roughly half the apparent fit does not
survive. \textbf{More than 180 days: 0.078 in, 0.054 out. More than 365 days: 0.045 in, 0.032
...
Over all cases, the residual standard deviation is \textbf{1.815 log points}
...
returning at all. Table~\ref{tab:returning} reports it: there $R^2 = 0.189$ and the
\textbf{residual standard deviation is 1.064 log points}, meaning a one-standard-deviation
