...
% (analyze_delay.py universe).
\input{tables/delay_macros}
...
\paragraph{Note, 2026-09-11: the case universe.} ``Upper-cased, whitespace stripped'' counts a
case printed with a two-digit year at one hearing and a four-digit year at another as two. The
universe every memo now uses (\texttt{case\_universe} in \texttt{acquire\_external\_data.py})
expands the year first and holds \delayCasesUniverse\ cases, not \delayCasesPrinted. Recomputed
both ways by \texttt{analyze\_delay.py universe}, the construction above reproduces on the
printed numbers (an estimation sample of \delaySamplePrinted\ cases, \delayMultiPrinted\% heard
on more than one day, \delayPositivePrinted\% with a positive elapsed time) and on the universe
gives \delaySampleUniverse, \delayMultiUniverse\% and \delayPositiveUniverse\%. The models
below were estimated on the printed numbers and have not been re-estimated on the universe; the
two samples differ only in the cases printed both ways. (The two shares in the paragraph
above are, as Table~\ref{tab:returning} says, shares of the estimation sample, not of all
cases.)
