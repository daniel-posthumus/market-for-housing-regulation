...
narrows a median-ten parcel history to a median-two candidate set at 0.70 precision and 0.92
recall.
...
structured fields, at 96.8\% field-level accuracy on a frozen test half. Nothing here
...
\textbf{\texttt{permit\_number} is not a key.} The 1{,}295{,}048 rows carry 1{,}149{,}253
...
2{,}909 such items whose cited permit is present at the parcel DBI records for it. Coverage
...
and signs --- raises precision to 0.43 at almost no cost in recall, and is the single most
valuable filter. Adding a date window trades recall for coverage: \textbf{filings from three
years before to one year after the hearing return a candidate set for 94.1\% of items, at
0.70 precision and 0.92 recall, with a median set size of two}. Tightening to one year before
...
Taking only the nearest filing gives point identification at 0.73 precision and 0.66 recall.
That is the right rule if the analysis needs one permit per item and can tolerate a third of
them being wrong; the three-year window with a median-two candidate set is the right rule if
...
number, 10{,}076 have a block and lot, 8{,}985 have a parcel DBI knows, and \textbf{6{,}323
--- 39.0\% of the whole docket --- get at least one candidate permit} under the preferred
rule. Combined with the 3{,}515 items matched on a printed number, some construction-side
observation is available for \textbf{9{,}838 items, 60.7\% of the docket}, of which a little
over a third is exact and the rest is a small candidate set with a known error rate.

This is a positive result but it should not be oversold. The 0.70 precision is measured on
...
\item A parcel-plus-window-plus-type rule extends partial coverage to a further 39\% of the
docket at 0.70 precision and 0.92 recall, validated against the permit-number subset.
