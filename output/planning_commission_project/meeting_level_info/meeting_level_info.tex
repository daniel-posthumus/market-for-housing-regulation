...
\medskip\noindent
The remaining error is not spread evenly. Scoring \emph{today's} rules against all
134 hand-confirmed meetings gives \textbf{93.9\%} of 938 field values, and the spread
by era is wide: \textbf{98.3\%} from 2015, \textbf{92.2\%} for 2002--2014 and
\textbf{91.0\%} for 1998--2001. The early archive wraps its header labels mid-phrase, and
that single formatting habit accounts for most of what is left; \texttt{staff}
(\textbf{86.6\%}) and \texttt{presiding} (\textbf{88.1\%}) are the two weakest fields.
That figure is produced by \texttt{extract\_all\_meetings.py --score} and regenerates with
the data, unlike the round figures above, which are historical measurements against code
states that no longer exist. It is also \emph{unadjudicated}: spot-checking the
disagreements turns up hand labels that are themselves wrong, so it is a lower bound.

\medskip\noindent
The validated extraction has now been run over the whole corpus --- 1{,}125 documents
yielding \textbf{1{,}219 meetings}, 1998--2026 --- with no hand-marking. Part III reports
what that data shows. The 2018 scraping gap that ran through the first version of this memo
was closed on 2026-09-04: the year now holds 42 meetings rather than two, and every figure
and table here is computed on the refilled corpus.
\end{abstract}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/extraction_accuracy.pdf}
...
\paragraph{The number that answers ``how accurate is this?''} There are two, and they
answer different questions. \textbf{96.0\%} is the \emph{generalisation} estimate: the share
of field values matching the hand labels on 14 meetings the rules had never seen, whose
\emph{boundaries the machine placed itself}. Out-of-sample accuracy rose at every round ---
92.1\%, then 95.3\%, then 96.0\% --- each measured before the rules saw the round in
question. \textbf{93.9\%} is the \emph{current} figure: today's rules scored against all
134 hand-confirmed meetings, 938 field values, by \texttt{extract\_all\_meetings.py
--score}. It is the lower of the two for three reasons worth keeping apart --- it covers
every round rather than the best one, it uses a stricter set-exact comparison on the list
fields, and its labels have not been adjudicated. Prefer 96.0\% when the question is ``will
this generalise?'' and 93.9\% when it is ``what is in \texttt{meetings\_all.csv} right
now?''
...
for the 135 hand-marked meetings and having it for all 1{,}219}. Round 4 then confirmed the
...
not just the hand-labelled sample: 1{,}125 documents, 1998--2026, producing
\textbf{1{,}219 meetings}. 134 of them are hand-verified; the rest carry the machine's
...
Field & Populated & \% \\
\midrule
\texttt{location}      & 1{,}219 & 100.0 \\
\texttt{meeting\_time} & 1{,}209 &  99.2 \\
\texttt{meeting\_type} & 1{,}204 &  98.8 \\
\texttt{present}       & 1{,}177 &  96.6 \\
\texttt{staff}         & 1{,}150 &  94.3 \\
\texttt{presiding}     & 1{,}145 &  93.9 \\
\texttt{absent}        &   553 &  45.4 \\
\bottomrule
\end{tabular}
\caption{Field coverage across all 1{,}219 meetings. \texttt{absent} is low because most
meetings record no absences, not because the field fails.}
\end{table}

\paragraph{A sanity check the data passes.} \textbf{1{,}202 of 1{,}219 meetings fall on a
Thursday} (98.6\%), the Commission's sitting day. The 17 that do not are 7 Tuesdays, 5
Mondays and 5 Fridays --- special and joint sittings, which is what one would expect. The
share is unchanged by the 2018 refill, whose 42 recovered meetings are all Thursdays. No
date was ever tuned to produce this.

\subsection{Joining an item to its meeting}

The meeting table is only useful if every item can find its row in it, and the obvious key
--- the date --- is not one. Seven archive-era documents hold two meetings on the same day:
the Commission sat jointly with another body and then in its own regular session, each with
its own roll call and adjournment. \textbf{203 items} sit in one of those documents, and a
date-only join would have put all 203 in whichever meeting happened to be indexed first,
silently mixing a joint sitting's attendance into a regular one and vice versa. The correct
split is 182 regular and 21 joint.

The key is therefore \textbf{(document, header ordinal)}: the index of the meeting header an
item falls under, counted in document order. Both sides derive it from the same
\texttt{header\_dates()} call over the same text --- \texttt{assign\_meeting\_dates.py}
stamps it on each item as \texttt{meeting\_ordinal}, \texttt{extract\_all\_meetings.py}
writes it on each meeting --- so the two enumerations are the same enumeration and the join
is exact rather than approximate. Modern-era items, whose parsed \texttt{tagged/} filenames
no longer correspond one-to-one with the raw documents, fall back to the date, and to the
archive's own filename token (\texttt{cal\_min} vs \texttt{jnthrgcpc\_min}) on the thirteen
days that hold two meetings.

\textbf{25{,}898 of 25{,}927 items (99.9\%) resolve to a meeting.} Only 29 do not: one
scraped page that returned a JavaScript stub instead of minutes (2016-04-21) and one
2020 \emph{corrected} minutes file with no raw counterpart. Those items keep
their dates, which come from their filenames; only the hearing-level attributes are missing,
and they are left blank rather than inferred.

This is the first number the 2018 refill moved, and it moved a long way. Before it, 781
items were unmatched and 759 of them were 2018: their minutes had been parsed on a machine
that was subsequently lost, so the blocks survived in \texttt{labels.db} while the documents
their headers would be read from did not. Recovering the PDFs took item coverage from
96.6\% to 99.9\% and gave 773 items a hearing --- 724 regular, 49 joint.

\begin{table}[h]
\centering
\begin{tabular}{lrr}
\toprule
Meeting type of the item's hearing & Items & \% \\
\midrule
\texttt{regular}        & 24{,}783 & 95.6 \\
\texttt{special}        &     512 &  2.0 \\
\texttt{joint}          &     377 &  1.5 \\
\texttt{closed\_session} &      63 &  0.2 \\
(type not read)         &     163 &  0.6 \\
\midrule
No meeting record       &      29 &  0.1 \\
\bottomrule
\end{tabular}
\caption{All 25{,}927 items, by the character of the hearing they were decided at.}
\end{table}

\paragraph{Where this shows up.} The item-level labeling app reads the join and shows the
meeting above the form --- date and weekday, meeting type, the other body when it is a joint
hearing, gavel time, room, and who sat. None of it is typed by hand, which is the point:
\texttt{meeting\_date} was removed from the item schema in August 2026 precisely because it
is a property of the hearing, and the meeting bar is what replaces it. Items whose meeting
could not be resolved say so in place of showing a guess.
...
\midrule
1998--2004 &  303 &  270 & 10 & 15 & 10.9\% \\
2005--2011 &  377 &  279 & 72 & 20 & \textbf{26.0\%} \\
2012--2018 &  263 &  231 &  5 & 18 & 12.2\% \\
2019--2026 &  276 &  261 &  1 & 14 &  5.4\% \\
\midrule
All        & 1{,}219 & 1{,}041 & 88 & 67 & 14.6\% \\
\bottomrule
\end{tabular}
\caption{Meeting type over time. 15 meetings (1.2\%) have no type recorded and 8 are closed
sessions. The 2012--2018 row is the one the 2018 refill changed: 41 more meetings, of which
3 are joint sittings and 1 special.}
\end{table}

\noindent
The striking pattern is the \textbf{special-meeting surge in 2005--2011}: 72 of the 88
special meetings in the entire corpus fall in that seven-year window --- 25 of them in 2007
and 17 in 2008 alone --- and non-regular sittings run at 26.0\% against 5--12\% in every
other period. Refilling 2018 did not touch it: the year contributed no special meetings. Whatever drove it --- the
...
The Commission sits roughly weekly: 27--66 meetings a year, with the low years being those
where the archive itself is thin (2015--2017 sit at 27--29). 2018, at 42, is now mid-range.

\subsection{Attendance}

Attendance is recorded for 1{,}177 meetings. Mean commissioners present is \textbf{6.19}
(median 6, of a seven-member Commission), and \textbf{45\% of meetings record at least one
absence}, 709 absences in total.
...
Fong             & 34 & 319 & 10.7\% \\
\midrule
\emph{``Lee'' (unresolved)} & 36 & 169 & 21.3\% \\
\bottomrule
\end{tabular}
...
meetings and William L. Lee 220, each with a single recorded absence. \textbf{169 meetings
...
should either drop those 169 meetings or resolve them by hand.

\subsection{Who presides, where, and when}

Fong presided over 137 meetings, Olague 125, Miguel 101, Theoharis 95, Bell 88,
Koppel 82, a Lee 73, Tanner 63, Chinchilla 62, Hillis 57 --- a genuine rotation rather than
a dominant chair. (Theoharis moved from 64 to 95 on 2026-09-09: a mojibake token in the
2000--2001 roll calls was registering as a second claimant of the surname, which stopped
``Anita Theoharis'' reducing to ``Theoharis'' and split one commissioner into two.)

\textbf{1{,}065 meetings sat at City Hall}, 51 at the War Memorial Building (the pre-2001
home), and \textbf{80 were remote hearings} --- the pandemic-era teleconference period, now
a clean flag. The remaining 23 (1.9\%) name none of the three. Two of those are real offsite
sittings --- 1600 Holloway Avenue (San Francisco State) and Mission High School --- and the
other 21 are extraction failures: page chrome, a bare date, or the literal word ``text''.

Start times cluster hard: 480 at 1:30 PM, 294 at 12:00 PM, 251 at 1:00 PM. The shift from
1:30 to 12:00 tracks the modern era.

\textbf{67 joint sittings} are recorded, most often with the Recreation and Park Commission
(16), Health (9 across two spellings), Building Inspection (8 across two spellings), Historic
Preservation (7 across two spellings) and the Redevelopment Agency (5 across two spellings).

\subsection{Two series over time}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/meeting_timeseries.pdf}
...
Chinchilla & 2002-01 -- 2002-05 &  16 &  9.8\% & 12.6 \\
...
Olague     & 2011-01 -- 2011-12 &  45 &  7.3\% &  9.8 \\
Miguel     & 2011-12 -- 2012-02 &  11 &  1.4\% &  9.3 \\
Fong       & 2012-03 -- 2014-03 &  90 &  7.3\% &  9.0 \\
Wu         & 2014-03 -- 2015-02 &  35 &  2.0\% & 10.3 \\
Fong       & 2015-03 -- 2017-01 &  55 &  8.8\% &  9.4 \\
\textbf{Hillis} & \textbf{2017-02 -- 2019-01} & \textbf{61} & \textbf{10.3\%} & \textbf{10.0} \\
Melgar     & 2019-01 -- 2019-12 &  35 & 12.5\% & 10.4 \\
Koppel     & 2019-12 -- 2022-01 &  83 &  6.1\% &  9.7 \\
Tanner     & 2022-01 -- 2024-01 &  71 &  6.7\% &  9.4 \\
Diamond    & 2024-02 -- 2024-09 &  21 &  4.4\% &  8.0 \\
So         & 2024-09 -- 2026-03 &  56 &  7.4\% &  8.6 \\
\bottomrule
\end{tabular}
\caption{All 19 inferred presidency terms. Absence rate is the mean share of the seated
Commission missing a sitting. \textbf{Hillis is new to this table}: the term spans 2018, and
while that year was a hole in the corpus the run fragmented below the six-meeting floor and
was absorbed into its neighbours. Recovering 2018 recovered a two-year presidency ---
a reminder that a scraping gap does not stay confined to the years it covers.}
\end{table}
...
the pattern is mostly chronological rather than personal: three of the four highest rates
are the three earliest presidencies (Melgar's 12.5\% in 2019 is the exception), and Fong's
two separate terms differ from each other (7.3\% then 8.8\%) by more than many presidents
differ from their neighbours. That is the signature of a
...
\medskip\noindent
Earlier versions of both panels broke at 2018, where the corpus held only two documents. The
year was refilled on 2026-09-04 and now carries 42 meetings, mid-range for the panel, so
both series run continuously from 1998 to 2026. The gap-drawing machinery stays in the plot
script with an empty list of sparse years: the rule --- draw a gap rather than a line
through one --- outlives the instance that prompted it.

\section{Reproducing this}

\begin{verbatim}
cd code/commission_minutes_processing
python minutes_scraping/scrape_minutes.py --year 2018  # refill 2018
python assign_meeting_dates.py --apply --sync-labels   # dates + ordinals
python meeting_headers.py --export                     # meeting fields + CSV

python extract_all_meetings.py                # all 1,219 meetings -> CSV + table
python plot_extraction_accuracy.py            # the accuracy figure
python plot_meeting_timeseries.py             # Figure 1
python draw_validation_sample.py --seed S --round N   # another round

cd date_boundary_app
python app.py                 # mark boundaries (:5006); /meetings confirms
...
\texttt{meetings\_all.csv} (all 1{,}219 extracted meetings, with an \texttt{era} column and
...
\item \textbf{The 2018 hole is closed, and what it cost is worth recording.} Until
2026-09-04 only two documents existed for 2018, against 27--44 for its neighbours. It was a
scraping gap, not a year the Commission did not sit: 40 further minutes PDFs were recovered
from the S3 packet prefix, taking the year to 42 meetings. While it was open it accounted
for 759 of the 781 unmatched items, suppressed a two-year Hillis presidency from the
inferred term calendar, and forced a drawn break through the middle of both time series.
A further 8 documents surfaced that \texttt{labels.db} has never held (roughly 200 items);
those are extracted at the meeting level but are not yet in the item-level queue, so any
item-level count for 2018 is still short by that much until the next ingest. Twelve 2018
Thursdays have no document at any URL --- New Year, Independence Day, the August recess,
Thanksgiving and Christmas weeks --- and are recess, not gap.
\item \textbf{Twenty-one meetings (1.7\%) carry an unusable venue string} --- page chrome,
a bare date, the literal word ``text'' --- and 15 (1.2\%) no meeting type. Two further
meetings name a venue that is real but not one of the three the pipeline recognises (1600
Holloway Avenue, Mission High School); those are correct data, not errors. All are visible in
\texttt{meetings\_all.csv} and should be dropped or repaired rather than analysed.
\item \textbf{The special-meeting surge of 2005--2011 is a finding, not yet an explanation.}
72 of the corpus's 88 special meetings fall in that window, 25 of them in 2007 alone. It could be substantive or an
...
\begin{longtable}{p{0.22\textwidth}p{0.10\textwidth}p{0.25\textwidth}p{0.12\textwidth}p{0.12\textwidth}}
\toprule
