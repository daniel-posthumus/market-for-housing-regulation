...
\medskip\noindent
\textbf{Fifth, on prices, the important finding is an absence.} The CoreLogic pull did
happen --- it is documented as complete, \acqCoreLogicSales\ San Francisco sales --- and
\textbf{the extracts are not under the data root}. That is a human action item, not a search
result. What is cached instead: Zillow ZHVI and ZORI for San Francisco, and the assessor's
assessed values with the Proposition~13 caveat stated. On fees, \acqFeeRegisters\ annual
...
\paragraph{CoreLogic / Cotality: the pull happened and the files are gone.} The brief asked
whether the pull went through before Stanford access lapsed on 1 July. It did.
\texttt{demand\_estimation/stubs.py} records the source's status as \texttt{landed} with a
full provenance note --- pulled from Stanford Libraries' Redivis ``Data Farm'', filtered
server-side to the Bay Area county FIPS, PII excluded at query time --- and the summary
tables it produced are still in the repository:
\acqCoreLogicSales\ San Francisco sales, with a median price and a median price per square
foot, and a parcel file carrying year built, bedrooms, bathrooms, living area, lot size,
assessed value and annual property tax.

\textbf{The extracts themselves are not under \texttt{\$MFHR\_DATA\_ROOT}.} Neither is the
rest of the \texttt{demand/} subtree the demand pipeline writes. The most likely explanation
is the machine loss that cost the corpus and the hand labels and from which the labels were
recovered by content-matching; the CoreLogic extracts had no such second copy. \textbf{This is
a human action item, not a search result}: either the Redivis extracts are re-exported (the
query templates and column choices are documented in the stub), or the demand-side price spine
is re-requested through BU. It should not be looked for again on disk.
...
\acqCuUnitsNZ\%.
\item The CoreLogic pull was completed and its output is absent from the data root.
...
\acqFeeFirst--\acqFeeLast. \textbf{No single window has all of them}, and a specification
...
\paragraph{Access questions left open.} CoreLogic (re-export from Redivis, or a BU request).
CoStar (BU Questrom holdings, unchecked by design). The \S415 inclusionary requirement
percentages and their amendment history. Fee registers before \acqFeeFirst.
...
two figures quoted from the brief itself (the $\geq$95\% acceptance
criterion and the 1 July access lapse); and three figures quoted from sibling memos, each
