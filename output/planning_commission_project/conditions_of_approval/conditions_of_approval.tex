...
% The figures quoted by the dated corrections of 2026-09-11 come from the memos that now own
% them, through tables/condition_macros.tex (written by analyze_conditions.py report).
\input{tables/condition_macros}
...
authorisation. \emph{(Correction, 2026-09-11: the ``0\% before 2010'' was a finding about two
URL patterns; a census of every case finds condition text for conditioned items heard from
\condEarliestItemYear, thin before 2010 but not absent. See the corrections in \S\ref{sec:complement} and \S\ref{sec:left}.)}
...
pattern, which is a \emph{time series}: the conditioning share rose from 13.4\% in 1998 to
45.5\% in 2023, and the content of the conditions is observable only over the second half of
that span. Any claim about how the \emph{content} of conditions changed will be a claim about
2011 onwards.

\paragraph{Correction, 2026-09-11: Table~\ref{tab:packets} is superseded.} The table and the
list above were a sample of \emph{two URL patterns}, and ``1998--2009: 0 of 240 probed.
Nothing. The packet archive does not reach back that far'' was a statement about those
patterns, not about the archive. The conditions-content memo replaces the sample with a census
of every case and motion number against every address the department has published a motion
at: \condProbed\ URLs asked for, \condProbeHits\ documents found. Adopted motions --- not
only draft packets --- survive on the old site's FTP tree from \condEarliestFtp, in its document
modules from \condEarliestModules, and in Wayback captures of the pre-2003 city site from
\condEarliestWayback; the earliest conditioned item that now has condition text was heard in
\condEarliestItemYear, and the oldest motion read dates from \condEarliestAny. That memo's census table is what this table becomes when every case is
asked about.
...
\paragraph{Correction, 2026-09-11.} The correction above cites the data-acquisition memo as
``(\S3.3)'' and gives the bridge as reaching ``46.0\% of the 5{,}074 conditional-use items''.
Both were right of that memo's draft and are wrong of its final version. The section is now
\S5.4, and the share is \textbf{\condBridgeCUPct\%} (\condBridgeCUItems\ of \condCUItems):
the figure moved when that memo's revision of 2026-09-08 (commit \texttt{11059d9}) began
expanding two-digit years, which lets pre-2002 conditional uses reach their records. And
``nearly half of the conditional-use docket'' is the bridge's reach, not the overlap: the
bridge reaches conditional uses of every year, condition text survives only for some. Measured
directly against the conditions census, \textbf{\condOverlapCU\ conditional-use items have both}
a permit through the bridge and condition text; \condCondTextCU\ have condition text at all.
...
sampled cases from 2011 on and none before 2010. \emph{(Correction, 2026-09-11: ``none before
2010'' was about the sampled URL patterns, not the archive; see \S\ref{sec:complement}.)}
...
\begin{enumerate}
\item \textbf{Probe every case, not twenty a year.} 8{,}987 cases\footnote{Correction,
2026-09-11: this counted the case numbers as printed. \condCaseMerges\ cases were printed both
ways (\texttt{99.530C} and \texttt{1999.530C}), and the case universe --- defined once, in
\texttt{case\_universe}, and used by every memo --- is \condUniverse\ cases, not
\condCasesPrinted. The census the item proposes has since been run, in the conditions-content
memo.}, of which the families that
...
\paragraph{Correction, 2026-09-11.} Item~5 above --- ``No amount of work recovers condition
content before 2010'' --- is withdrawn. It generalised from two URL patterns to the archive.
The census found adopted motions from before 2010 on the old site and in the Wayback Machine,
and adopted motions attached to later packets; condition text now exists for
\condReachEarlyHtml\% of conditioned items heard in 1998--2002 and \condReachLateHtml\% in
2003--2009, against \condReachEarlyPdf\% in 2011--2014. The corrected statement is that pre-2010
condition text is \emph{thin}, not absent, and that the residual is documents not found at any
address --- \condResPreTenNone\ of the \condResPreTen\ pre-2010 conditioned items without text
--- rather than documents found and unread. The measure is still weighted towards 2011 onward,
and the earlier half of the corpus will still be carried mostly on the flag.
