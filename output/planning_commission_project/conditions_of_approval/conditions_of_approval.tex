...
and \emph{names} each condition. It is a conditional-use instrument: in a pilot pull of 146
packets, 55 of 64 conditional-use cases yield an Exhibit A with a median of fifteen named
conditions, and all 28 discretionary reviews yield none, because a DR produces a Discretionary
Review Action rather than a conditioned authorisation.

\medskip\noindent
\textbf{The conditions are templated, and a small scheme spans them.} The 146 packets yield
166 distinct condition headings over 928 impositions, and a thirteen-category keyword scheme
--- split into conditions that change the \emph{project} and conditions that change its
\emph{obligations} --- reaches \textbf{84.9\% of those impositions} and 72.3\% of the
\texttt{modifications} field, the one place the existing dataset already holds condition
content. The split between the families is the sharpest result in the memo: weighted by how
often they are imposed, \textbf{62\% of staff's conditions change the project's obligations
and 15\% change the project}, while in the Commission's own amendments the proportions are
reversed.
\end{abstract}
...
Table~\ref{tab:kinds} reports a pilot pull of \textbf{146 packets}, of which 61 carried an
Exhibit A. The split by kind of case is clean and it is not a parser artefact:

\begin{itemize}
\item \textbf{Conditional use: 55 of 64}, with a median of \textbf{fifteen} named conditions.
\item \textbf{Discretionary review: 0 of 28.} Their packet is a ``Discretionary Review
...
motion''.
\item \textbf{Variance 0 of 5, environmental 0 of 7, legislative 0 of 14.} None of these ends
in a conditioned authorisation either.
\end{itemize}

This is the mirror image of the permit-linkage result, and the two memos should be read
together. \textbf{The permit route reaches discretionary review (83.8\% of DR items match a
DBI permit) and essentially nothing else; the conditions route reaches conditional use and
essentially nothing else.} Between them they cover most of the docket, but they never cover
the same item, so an analysis that wants both construction outcomes and condition content on
one project cannot have them from these two sources.
...
Table~\ref{tab:titles} lists the most frequent condition headings across the packets pulled.
They yielded \textbf{166 distinct headings over 928 impositions} --- 15 conditions per motion,
drawn from a vocabulary of under two hundred. The template is immediately visible.
...
Applied to the two sources of text, the scheme reaches \textbf{84.9\% of the 928 condition
impositions} in the packets and \textbf{72.3\% of the 1{,}735 \texttt{modifications} texts}.
Coverage of \emph{distinct} headings is much lower, 58.4\%, and the gap is informative rather
than embarrassing: 45 of the 69 uncovered headings appear exactly once. The template is a small
...
\textbf{62.3\% of the packet conditions change the project's obligations and 15.0\% change the
...
\textbf{0.0\%} of packet condition headings and 28.5\% of modifications. Staff's conditions
do not name heights and setbacks at all --- the building's shape is fixed by the plans in
Exhibit B, not by a named condition --- while a third of what the Commission adds from the
dais is exactly that. \textbf{Staff write the obligations; the Commission reshapes the
building.} If that holds on a full pull it is a finding about who exercises which margin of
...
(21.7\% of impositions) and \emph{validity and timing} (19.5\%) --- enforcement, revocation,
expiry, extension, diligent pursuit. Two fifths of the conditions a project receives are about
...
\section{What this establishes, and what a pilot would need}
...
\item The Commission packet does, addressably, by case number on two hosts --- for roughly two
thirds of cases from 2011 on and none before 2010.
\item The packet route is a conditional-use instrument. Discretionary review has no motion and
no Exhibit A. This is the exact complement of the permit route.
...
\textbf{What a full pilot would need.} The pull done here is a sample sized to answer the
availability question, not to build the measure. To go from here to an indicator matrix:

\begin{enumerate}
\item \textbf{Enumerate rather than sample.} 8{,}987 cases, of which the conditional-use family
heard from 2011 on is the addressable set; probe all of them and pull what exists. The probe is
one HEAD request per case and costs minutes; the pull is the expensive step, at roughly eight
megabytes and several seconds a packet, and it should be run once and cached, as the two
network stages of \texttt{analyze\_conditions.py} already are.
...
stringency available and one nothing in the current dataset can compute.
