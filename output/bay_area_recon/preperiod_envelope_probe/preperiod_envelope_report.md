...
> **CORRECTION (supersedes the counts below): the usable pre-period count is 15/25 (60%), not 17/25.** A later verification pass killed two sources — **San Ramon** (`rezoning.pdf` is a P-1 rezoning *application* doc, not the citywide zoning map) and **Daly City** (the 2014 "Zoning Ordinance.pdf" sits under a General-Plan-Update path and could not be confirmed adopted) — both now `none_found` in `preperiod_envelope.csv`. Usable = 11 `ordinance_text` + 3 `zoning_pdf` + SF; `none_found` = 9. See `../memo.tex` (§4.3) for the current picture.
